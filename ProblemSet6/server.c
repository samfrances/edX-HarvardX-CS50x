...
char* indexes(const char* path)
{
    char* html_path = malloc(strlen(path) + strlen("index.html") +1);
    char* php_path = malloc(strlen(path) + strlen("index.php") +1);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    if (file != NULL) {
        ...
        int buff_size = 2000; // Initial buffer size
        int content_size = 0;
        int chunk_size = 1000; // Size of each chunk read in
        char* buffer = malloc(buff_size);
        
        while ( true ) {
            // Resize buffer if necessary
            if (buff_size < content_size + chunk_size) {
                buff_size *= 2;
                buffer = realloc(buffer, buff_size);
            }
            
            // Read in a chunk, update size, and check we haven't reached the end
            int bytes_read = fread( buffer + content_size, sizeof(BYTE), chunk_size, file );
        ...
        buffer = realloc(buffer, content_size);
...
{
    memset(abs_path, '\0', LimitRequestLine);
    memset(query, '\0', LimitRequestLine);
    
    int verb_end = 0; // current index
    ...
    if (strncmp(line, "GET", verb_end) != 0) {
        error(405);
        return false;
    }
    
    // Check request-target
    int req_start = verb_end += 1;
    int req_end = req_start;
    int query_mark = 0;
    
    // Error if the path doesn't start with '/'
    if (line[req_start] != '/') {
        error(501);
        return false;
    }
    
    // Run through request-target, checking
    // For illegal characters, locating
    // Any query string, and the end of request-target
    while (line[req_end] != ' ') {
        if (line[req_end] == '"') {
            error(400);
            return false;
    ...
    // Check HTTP version
    int http_start = req_end + 1;
    if (strcmp(line + http_start, "HTTP/1.1\r\n") != 0) {
        error(505);
        return false;
    }
    ...
    // Return error if path too long
    int len_path = path_end - req_start;
    if (len_path > LimitRequestLine) {
        error(400);
        return false;
    } 
    strncpy(abs_path, line + req_start, len_path);
    
    // Copy out query
    
    // Return error if path too long
    int len_query = req_end - (query_mark + 1);
    if (len_query > LimitRequestLine) {
        error(400);
        return false;
    }
    // Copy out query
    if (query_mark > 0) {
        strncpy(query, line + query_mark + 1, len_query);
    } else {
        query[0] = '\n';
    }
